...
public class Throwable3 : MonoBehaviour
{
    ...
    [Header("Input")]
    [SerializeField] private InputActionReference werfenAction;
    ...
    private InputAction fallbackWerfenAction;
    ...
    {
        EnableAction(werfenAction);
        fallbackWerfenAction?.Enable();
    }
    ...
    private bool WasWerfenPressed()
    {
        if (werfenAction != null &&
            werfenAction.action != null &&
            werfenAction.action.WasPressedThisFrame())
            return true;

        if (fallbackWerfenAction != null &&
            fallbackWerfenAction.WasPressedThisFrame())
            return true;
    ...
    private void CreateFallbackWerfenAction()
    {
        fallbackWerfenAction = new InputAction(
            "Throw Held Object Fallback",
            InputActionType.Button);

        fallbackWerfenAction.AddBinding("<XRController>{RightHand}/{GripButton}");
        fallbackWerfenAction.AddBinding("<Mouse>/rightButton");
    }
