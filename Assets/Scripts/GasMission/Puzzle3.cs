...
public class Puzzle3 : MonoBehaviour
{
    ...
    [SerializeField] private string missionTutorialText =
        ...
        "Druecke [T], um sie ein- oder auszublenden.";
    ...
    private IEnumerator ShowTutorialSequence()
    {
        ...
        tutorialManager.ShowTutorial(
            GetPauseTutorialText(),
            pauseTutorialDuration
        );
    }

    private string GetMissionTutorialText()
    {
        if (!VRUIRuntimeSupport.IsLikelyVrScene())
            return missionTutorialText;

        return "Du kannst deine aktuellen Missionsziele jederzeit einsehen.\n\n" +
               "Druecke einen der linken Controller-Buttons, um sie ein- " +
               "oder auszublenden.\n\n" +
               "Mit dem linken Grip blendest du das HUD ein und aus.";
    }

    private string GetPauseTutorialText()
    {
        if (!VRUIRuntimeSupport.IsLikelyVrScene())
            return pauseTutorialText;

        return "Du kannst das Spiel jederzeit pausieren.\n\n" +
               "Druecke dazu den Menu-Button am linken Controller.";
    }
